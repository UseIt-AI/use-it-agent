...
import { Monitor, Box, Check, Plus, Trash2 } from 'lucide-react';
...
export function AgentTargetList({
  ...
  const handleClick = (target: AgentTarget) => {
    if (selectedId === target.id) {
      // 再次点击已选中的项 -> 不做任何操作 (之前是取消选择回到 Overview)
      return;
    } else {
      onSelect(target);
    }
    ...
    <div className="w-[280px] flex flex-col border-l border-divider bg-canvas-sub/30 relative">
      ...
      <div className="flex-1 overflow-y-auto">
        {targets.map(target => {
          ...
          const isVm = target.type === 'vm';
          const Icon = isVm ? Box : Monitor;
          // VM 即使尚未安装（available=false）也允许点击进入（用于安装/连接）
          const canSelect = isVm ? true : target.available;
          
          // 状态颜色
          const dotColor = 
            target.status === 'running' ? 'bg-emerald-500' :
            target.status === 'off' ? 'bg-neutral-400' :
          ...
          return (
            ...
            >
              ...
              {/* 图标：无背景，纯线条 */}
              <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isSelected ? 'text-black' : 'text-black/40'}`} strokeWidth={1.5} />
              ...
              <div className="flex-1 min-w-0 flex items-center justify-between mr-2">
                ...
                {/* Hover Status Text */}
                <span className="text-[9px] font-mono uppercase tracking-wide opacity-0 group-hover:opacity-50 transition-opacity">
                  {target.status === 'running' ? 'READY' : target.status || 'READY'}
                </span>
              </div>

              {/* 右侧：Delete + Active */}
              <div className="flex items-center gap-1">
                {/* Delete (VM only, not This PC) */}
                {target.type === 'vm' && target.deletable !== false && (
                  <button
