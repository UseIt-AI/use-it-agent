...
| 方式 | 说明 | 适用场景 |
|------|------|----------|
| **connect** | 启动新浏览器（复制 Profile 到临时目录） | 需要独立浏览器实例，不影响用户正在使用的浏览器 |
| **attach** | 接管用户已打开的浏览器（通过 CDP） | 需要使用用户当前浏览器的登录态、Cookie 等 |

### 1.3 与 Computer Use (GUI) 的区别
...
### 1.4 使用 attach 接管浏览器的前置条件

用户需要使用以下命令启动 Chrome/Edge，开启 CDP 调试端口：
...
> **说明**: 单例模式下，只有一个全局浏览器连接。适合简单场景。
> 如需多实例，请使用 3.1 节的 Session 管理。

#### `connect` - 启动新浏览器并连接

启动一个新的浏览器实例（复制 Profile 到临时目录，不影响用户正在使用的浏览器）。
...
```json
{
  ...
  "args": {
    ...
    "profile_directory": "Default", // 可选: 浏览器 Profile 名称
    "headless": false,            // 可选: 是否无头模式，默认 false
    "highlight_elements": true    // 可选: 是否高亮可交互元素，默认 true
  }
}
```
...
通过 CDP 连接到用户已经打开的浏览器（保留登录态、Cookie 等）。

⚠️ **前置条件**: 用户需要用 `--remote-debugging-port=9222` 启动浏览器，见 1.4 节。
...
```json
{
  ...
  "args": {
    ...
    "highlight_elements": true           // 可选，默认 true
  }
}
```
...
断开与浏览器的连接（如果是 connect 模式，会关闭浏览器；如果是 attach 模式，浏览器保持运行）。
...
### 3.1 Session 管理（多实例模式）

> **说明**: Session 模式下，可以同时管理多个独立的浏览器实例。
> 每个 Session 有唯一的 `session_id`，后续操作需要传入此 ID。
...
| Action | 必填参数 | 可选参数 | 说明 |
|--------|---------|---------|------|
| `connect` | - | `browser_type`, `profile_directory`, `headless`, `highlight_elements` | 启动新浏览器并连接 |
| `attach` | - | `cdp_url`, `highlight_elements` | 接管用户已打开的浏览器 |
| `disconnect` | - | - | 断开浏览器连接 |
| `status` | - | - | 获取连接状态 |

### 9.2 Session 管理（多实例模式）
