/**
 * Environment actions — AI-invocable helpers for active environment and workflows.
 */
import { z } from 'zod';
...
appAction.registerAction({
  ...
  description: 'List configured environments (This PC entries). Returns id, type, and name.',
  handler: async () => {
    ...
    const envs = ((config?.environments as any[]) || []).filter((e: any) => e?.type === 'local');
...
appAction.registerAction({
  ...
  description: 'Switch the active environment (agent target) using its environment id (e.g. "local" for This PC).',
  parameters: z.object({
