...
const ControlPanel = forwardRef<ControlPanelRef, ControlPanelProps>(({
    ...
    const loadEnvs = async () => {
      try {
        ...
        if (window.electron?.getAppConfig) {
          ...
          const raw = (fullConfig?.environments as any[]) || [];
          loaded = raw.filter((e: any) => e?.type === 'local') as EnvironmentConfig[];
        }
  ...
  useEffect(() => {
    const reload = async () => {
      try {
        ...
        let loaded: EnvironmentConfig[] = ((await window.electron.getAppConfig('environments')) as any[] || []).filter(
          (e: any) => e?.type === 'local'
  ...
  const targets: AgentTarget[] = useMemo(() => {
    return envs.map((e) => ({
      id: e.type === 'local' ? 'local' : e.id,
      type: 'local' as const,
      name: e.type === 'local' ? 'This PC' : e.name,
  ...
  const handleDeleteEnvironment = (_targetId: string) => {
    /* VM environments removed — no-op */
  };
