            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className={`text-[15px] font-bold ${error ? 'text-red-900' : 'text-black/90'}`}>
                  {error ? t('workspace.upload.titleFailed') : t('workspace.upload.title')}
                </h3>
                <p className={`text-xs mt-1 ${error ? 'text-red-600/70' : 'text-black/50'}`}>
                  {error 
                    ? t('workspace.upload.subtitleError') 
                    : t('workspace.upload.subtitle', { uploaded: uploadedCount, total: totalCount })}
                </p>
              </div>
            </div>
            {(onCancel || onClose) && (
              <button
                ...
                title={error ? t('workspace.upload.close') : t('workspace.upload.cancelUpload')}
              >
          ...
          {error ? (
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-red-800 mb-1 uppercase tracking-wide">
                      {t('workspace.upload.errorDetails')}
                    </p>
              ...
              {totalCount > 0 && (
                <div className="pt-3 border-t border-black/5">
                  <div className="flex items-center justify-between text-xs text-black/40">
                    <span>{t('workspace.upload.subtitle', { uploaded: uploadedCount, total: totalCount })}</span>
                    <span className="font-semibold">{progress.toFixed(0)}%</span>
                  </div>
                </div>
              )}
              
              {/* 关闭按钮 - Quick Start style */}
              <div className="flex justify-end pt-2">
                ...
                >
                  <span>{t('workspace.upload.close')}</span>
                </button>
              </div>
            </div>
          ) : (
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-black/40 font-medium">
                  {uploadedCount} / {totalCount} {t('workspace.upload.files')}
                </span>
              </div>
